...
def build_fake_deps(
    *,
    ...
    accessible on the returned object for call-order and argument
    assertions.
    """
    ...
    return CycleDependencies(
        config=FakeConfigRepository(resolved_config),
        warnings=FakeWarningProvider(resolved_warnings),
        forecast=FakeForecastProvider(resolved_forecast),
        composer=FakeMessageComposer(),
        contacts=FakeContactRepository(resolved_contacts),
        notifier=FakeNotifier(),
        alerts=FakeAlertRepository(alerts=list(prior_alerts or []), active_outage=active_outage),
